...
class populador:
    def __init__(self):
        ...
        for idClinica in range(21):
            texto = self.gerador(id, idClinica % 3)
            file.write(texto)
            id += 1
        file.close()

    def gerador(self, idContacto, idClinica):
        conector = ["do", "da", "de", ""]
    ...
    def criaContacto(self, idContacto, nome, apelido, initContacto, emailDomains):
        ...
        return f'''insert into CaumPetClinic.Contacto (idContacto, telemovel, telefone, email)
values("{idContacto}", "{contacto}", null, "{email}")'''
    ...
    def geraFuncionario(self, nome, idClinica, idMorada, idContacto):
        ...
        return f'''insert into CaumPetClinic.Funcionario (NIF, nome, dataDeNascimento, cargo, avaliacao, Clinica_idClinica, Morada_idMorada, Contacto_idContacto)
values("{nif}", "{nome}", DATE("{dataDeNascimento}"), "Estafeta", null, {idClinica},{idMorada},{idContacto})'''
