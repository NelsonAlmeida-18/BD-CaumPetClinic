...
criaPedido:BEGIN
    ...
    DECLARE CONTINUE HANDLER 
        ...
        if v_NIFFuncionario is NULL then
			SELECT Funcionario.NIF into v_NIFFuncionario FROM Funcionario where Funcionario.Clinica_idClinica=IdClinica limit 1;
        end if;
...
drop procedure if exists topClientesMensaisCadeia;
Delimiter $$
create procedure topClientesMensaisCadeia()
	BEGIN
        ...
        INNER join Pedido
        on MONTH(pedido.dataEfetivaRecolha)=MONTH(NOW()) 
...
drop procedure if exists topClientesMensaisClinica;
Delimiter $$
...
drop procedure if exists topAnimais;
delimiter $$
...
drop function if exists criaCliente;
delimiter $$
...
drop function if exists criaAnimal;
Delimiter $$
create function criaAnimal(nome varchar(100), peso FLOAT, dataDeNascimento DATE, especie VARCHAR(100), raca varChar(100), historico varchar(200), NIFDONO INT)
    ...
    begin
        ...
        insert into Animal(idAnimal,nome, peso, dataDeNascimento, especie, raca, historico, Cliente_NIF) values (vIdAnimal,nome,peso, dataDeNascimento, especie, raca, historico, NIFDONO);
